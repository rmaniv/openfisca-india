...
These elements are described in different folders. All the modelling happens within the `openfisca_india` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-india.git
cd openfisca-india
...
Substitute your package's country name for `openfisca_india` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_india/situation_examples/couple.json \
